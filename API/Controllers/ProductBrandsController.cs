...
namespace API.Controllers
{
    public class ProductBrandsController : BaseApiController
    {
        ...
        public ProductBrandsController(StoreContext context) => _context = context;
        ...
        public async Task<ActionResult<IReadOnlyList<LookupEntityDto>>> List([FromQuery] bool includeInactive = false, CancellationToken cancellationToken = default)
        {
            ...
            var query = _context.ProductBrands.AsNoTracking().AsQueryable();
        ...
        {
            var entity = await _context.ProductBrands.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id && x.IsUsed, cancellationToken);
        ...
        {
            var entity = new ProductBrand { Name = dto.Name.Trim(), IsUsed = dto.IsUsed };
        ...
        {
            var entity = await _context.ProductBrands.FindAsync(new object[] { id }, cancellationToken);
        ...
        public async Task<IActionResult> Delete(int id, CancellationToken cancellationToken)
        {
            var entity = await _context.ProductBrands.FindAsync(new object[] { id }, cancellationToken);
            if (entity == null) return NotFound();
            if (await _context.Products.AnyAsync(x => x.ProductBrandId == id && x.IsUsed, cancellationToken))
                return Conflict("Deactivate active products using this product brand first.");
