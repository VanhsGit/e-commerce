...
import { ADMIN_RESOURCES, AdminFieldConfig, AdminResourceConfig } from '../admin-resource.config';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NzButtonModule, NzFormModule, NzInputModule, NzInputNumberModule, NzModalModule, NzSelectModule, NzSwitchModule, NzTableModule, MetadataEditorComponent, EntityImageManagerComponent],
...
export class AdminEntityPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.config = ADMIN_RESOURCES[resource];
    for (const field of this.config.fields.filter(field => field.type !== 'metadata')) {
      ...
      this.form.addControl(field.name, new FormControl(initial, field.required ? Validators.required : []));
    }
    this.load();
  }

  get editableFields(): AdminFieldConfig[] { return this.config.fields.filter(field => field.type !== 'metadata'); }
  get hasMetadata(): boolean { return this.config.fields.some(field => field.type === 'metadata'); }
  get rowLabel(): string {
  ...
  load(): void {
    this.loading = true;
    this.http.get<Record<string, any>[]>(`${environment.apiUrl}${this.config.endpoint}?includeInactive=true`).subscribe({
      next: rows => this.rows = rows,
      error: () => { this.loading = false; this.messages.error(`Không tải được ${this.config.title}`); },
  ...
  save(): void {
    ...
    try {
      for (const field of this.editableFields.filter(field => field.type === 'json')) payload[field.name] = JSON.parse(payload[field.name]);
    } catch {
      this.messages.error('JSON không hợp lệ.');
      return;
    }
    if (this.hasMetadata) payload['metadata'] = this.metadata;
    if (this.editing && (this.config.entityType === 'ElectricBikeProduct' || this.config.entityType === 'AgriculturalMachineProduct')) payload['id'] = this.editing['id'];
    ...
    request.subscribe({
      next: () => { this.messages.success('Đã lưu dữ liệu'); this.modalOpen = false; this.load(); },
      error: error => { this.saving = false; this.messages.error(this.errorText(error)); },
  ...
  deactivate(record: Record<string, any>): void {
    ...
    this.http.delete(`${environment.apiUrl}${this.config.endpoint}/${record['id']}`).subscribe({
      next: () => { this.messages.success('Đã ngừng sử dụng'); this.load(); },
  ...
  reactivate(record: Record<string, any>): void {
    ...
    this.http.put(`${environment.apiUrl}${this.config.endpoint}/${record['id']}`, payload).subscribe({
      next: () => { this.messages.success('Đã kích hoạt lại'); this.load(); },
      error: error => this.messages.error(this.errorText(error)),
    });
  }
